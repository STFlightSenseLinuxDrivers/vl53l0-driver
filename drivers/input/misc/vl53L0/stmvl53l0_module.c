				...
				struct device_attribute *attr, char *buf)
{
	...
	pr_err("Driver Config:UseCase:%d, offsetCalDistance:%u,xtalkCalDistance:%u,setCalibratedValue:0x%X\n",
			vl53l0_dev->useCase, vl53l0_dev->offsetCalDistance, vl53l0_dev->xtalkCalDistance, vl53l0_dev->setCalibratedValue);
	...
	if (vl53l0_dev->useCase == USE_CASE_CUSTOM) {
		pr_err("CustomUseCase : Sigma=%u :Signal=%u: Pre=%u :Final=%u\n",
									...
									vl53l0_dev->preRangePulsePeriod, vl53l0_dev->finalRangePulsePeriod);	
...
static int stmvl53l0_flush(struct file *file, fl_owner_t id)
{
	...
	(void) file;
	(void) id;
	/* Revisit : Check if the instance are opened multiple times on some platforms*/
	mutex_lock(&data->work_mutex);
	if (data) {
		if (data->enable_ps_sensor == 1) {
...
static int stmvl53l0_init_client(struct stmvl53l0_data *data)
{
	...
	if (data->reset) {
		if (vl53l0_dev->setCalibratedValue & SET_XTALK_COMP_RATE_MCPS_MASK) { /* Xtalk calibration done*/
			...
			if (vl53l0_dev->XTalkCompensationRateMegaCps < 7*65536/10000) { //0.7 KCps converted to MCps
				ritValue = 15 * 7 * 65536/100000;
			} else {
				ritValue = 15 * vl53l0_dev->XTalkCompensationRateMegaCps/10;
			}
...
static int stmvl53l0_config_use_case(struct stmvl53l0_data *data)
{
	...
	vl53l0_dbgmsg("Configure UseCase(%d) : Sigma=%u,Signal=%u,Pre=%u,Final=%u,timingBudget=%u\n",
	...
	init_mode_e mode)
{
	...
	if (mode == OFFSETCALIB_MODE) {
		...
		*/
		...
		vl53l0_dev->XTalkCompensationRateMegaCps = XTalkCompensationRateMegaCps;
		vl53l0_dev->setCalibratedValue |= SET_XTALK_COMP_RATE_MCPS_MASK;
